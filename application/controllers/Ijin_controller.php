...
class Ijin_controller extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('Persetujuan_model');  // Pastikan model yang benar digunakan
    }
    ...
    public function filter_by_kelas() {
        ...
        try {
            ...
            if (!empty($pending_list)) {
                ...
                echo '<tr><td colspan="5" class="text-center">Tidak ada data tersedia untuk kelas yang dipilih.</td></tr>';
            }
        } catch (Exception $e) {
            log_message('error', 'Error pada filter_by_kelas: ' . $e->getMessage());
            echo '<tr><td colspan="5" class="text-center">Terjadi kesalahan saat memuat data.</td></tr>';
        }
    }
}
